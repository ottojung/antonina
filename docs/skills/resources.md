...
`antonina agent clean` is a different thing and is not a host-resource collector today: it is a manual sweep of Antonina's own durable agent state for agents in a terminal state older than a retention period.
