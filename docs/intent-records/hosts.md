...
What a re-check issues is the authority to act on the one path it read, not a token its holder may re-point: the fields an authorization carries are not frozen, and a caller still holding a writable reference to it may assign to them, but every consumer of the authorization re-derives what it needs from the module-private record the re-check left behind rather than from the caller's object, so a destructive action that finds any of them changed after the re-check is refused before anything is removed, not performed against the re-pointed values and refused afterwards.
