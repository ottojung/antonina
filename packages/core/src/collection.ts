 ...
 * re-check recorded, and only those -- and the caller's key set must match it in
 * length, so neither an added field nor a deleted one passes.
 *
...
function isUnchanged<T extends object>(authorized: T, issued: T): boolean {
  ...
  if (Object.keys(authorized).length !== carried.length) return false;
