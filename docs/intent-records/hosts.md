...
What a re-check issues is the authority to act on the one path it read, not a token its holder may re-point: the fields an authorization carries are held fixed at the destructive step rather than frozen at issue, and a destructive action that finds any of them changed after the re-check is refused rather than performed against the re-pointed values.
