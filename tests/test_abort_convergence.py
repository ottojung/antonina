...
def _spawn_ready_child(
    *,
    ...
    When *aid* is ``None`` no ``ANTONINA_AGENT_ID`` or invocation markers are
...
class _MarkedProcess:
    ...
    def __init__(self, aid: str, iid: str | None = None) -> None:
        ...
        if not agent.env_has_marker(pid, aid):
            self.kill_and_reap()
            pytest.fail(f"process {pid} missing ANTONINA_AGENT_ID={aid!r}")
...
def test_proven_scan_incomplete_after_member_keeps_and_closes_fds(
    ...
    iid = "b2d47eaebc8b95c78f28b5fe82bc8adc"
    marker_env = f"ANTONINA_AGENT_ID=aaaaaaaa\0{agent.INVOCATION_ID_VAR}={iid}\0".encode()
    pin_fd = os.open(os.devnull, os.O_RDONLY)
    try:
        payloads = {
...
def test_group_alive_stays_true_when_live_leader_environ_unreadable(
    ...
    try:
        ...
        def readable_foreign_environ(_self: Path) -> bytes:
            if "environ" in str(_self):
                return b"ANTONINA_AGENT_ID=other\0"
            return original_read_bytes(_self)
