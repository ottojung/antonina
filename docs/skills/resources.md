...
**The window is not bounded by the final component: any component can change, and the containing directory is the dangerous one.** Between the re-check's read and the removal, the final component can be swapped (to a symlink, which removes only the link — the safe direction; or to a directory, which is removed recursively). The *containing directory* can be replaced by a symlink too, and the removal then follows it and recursively deletes a tree outside the managed root — a tree the re-check never judged at any component. This cannot be closed: every check is a read followed by its own window, because neither the filesystem nor the board log offers a compare-and-remove. Keep the interval as short as you can, and treat a path that became protected after the re-check's read as a real possibility rather than an unlikely one. Recursive removal also descends into a bind or mount point inside the candidate — a devcontainer or sshfs mount under a worktree is realistic — and removes the contents from the mounted side; that is `rm -rf` semantics, not a collector guarantee.
