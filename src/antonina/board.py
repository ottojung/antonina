...
BOARD_BASE_URL_ENV: Final = "ANTONINA_BOARD_URL"
...
def _connection(
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        msg = "ANTONINA_BOARD_URL must be an absolute http:// or https:// URL"
        raise BoardError(msg)
    try:
        port = parsed.port
    except ValueError as exc:
        msg = "ANTONINA_BOARD_URL contains an invalid port"
        raise BoardError(msg) from exc
...
def _parser() -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(prog="antonina board")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        _write_stderr(f"antonina board: {exc}")
        return 1
