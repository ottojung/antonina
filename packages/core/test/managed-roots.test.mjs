...
test('an eligible candidate hands back one actionable path and no other', () => {
  ...
  // The root handed back is the very instance the validated set holds, and that
  // instance cannot be changed afterwards, so a collector cannot learn a root by
  // identity and then have it altered under it.
...
test('each candidate is judged only against the root it belongs to', () => {
  ...
  // The validated set keeps the configured order, because the first matching
