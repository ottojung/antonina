...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => { setView('issues'); setSelectedNumber(undefined); }} aria-label="Back to all Antonina issues"><span className="brand-mark" aria-hidden="true">B</span><span><strong>Antonina</strong><small>Shared issue board</small></span></button>
    ...
    <main className={`workspace ${view}-view ${selected ? 'has-selection' : ''}`}>
      ...
      {view === 'issues' ? selected ? <Thread issue={selected} hasWriteAccess={hasWriteAccess} displayName={displayName} setDisplayName={setDisplayName} saveDisplayName={saveDisplayName} openSettings={() => setSettingsOpen(true)} comment={postComment} editBody={(body) => run(() => api.editIssueBody(selected.number, body), 'Description updated')} close={() => void run(() => api.close(selected.number), 'Issue closed')} reopen={() => void run(() => api.reopen(selected.number), 'Issue reopened')} back={() => setSelectedNumber(undefined)} />
        : <section className="thread welcome"><div className="welcome-mark" aria-hidden="true">B</div><p className="eyebrow">Shared issue board</p><h2>{board!.issues.length ? 'Choose an issue to join the conversation.' : 'Start a shared record of the work.'}</h2><p>Issue descriptions hold the task context; conversation holds updates and questions.</p></section> : null}
    </main>
