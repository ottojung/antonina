"""Crash-durable filesystem primitives for Antonina authoritative local state.

Antonina uses these helpers when a state transition must survive a crash before
process lifecycle work may proceed. Regular files are written through a unique
same-directory temporary file, fsynced, atomically replaced, and confirmed by
fsyncing the parent directory. Symlink switches and removals use the equivalent
rename/unlink plus directory-fsync boundary.

Same-path operations are serialized across threads and processes so fail-closed
cleanup cannot overwrite a newer committed state. Confirmation failures raise
:class:`DurabilityError`; callers must not advance lifecycle authority when a
write or removal could not be confirmed durable.

The module also exposes deterministic fault-injection hooks used by tests.
"""
...
def write_bytes_durable(
    ...
    *,
    ...
    Args:
        ...
        data: Bytes to store.
        create_parent: Create and durably anchor the parent directory when it is
            absent. If false, the parent must already be an accessible directory.
    ...
    if create_parent:
        ...
        try:
            ...
            msg = f"cannot inspect destination directory {destination.parent}: {exc}"
            raise DurabilityError(msg) from exc
        if not stat.S_ISDIR(parent_mode):
            msg = f"destination parent is not a directory: {destination.parent}"
            raise DurabilityError(msg)
...
def write_text_durable(path: Path, text: str, *, create_parent: bool = True) -> None:
    """Crash-durably write ``text`` to ``path`` as UTF-8.

    Args:
        path: Destination regular-file path.
        text: Text to store.
        create_parent: Create and durably anchor the parent directory when absent.
