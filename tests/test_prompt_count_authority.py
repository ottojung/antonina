...
def test_canonical_prompt_count_increments(
    tmp_path: Path,
    initial: int,
) -> None:
...
def test_missing_prompt_count_blocks_acceptance(tmp_path: Path) -> None:
    """A missing current-schema counter is malformed, not an implicit zero."""
    meta = agent.idle_meta("audit", str(tmp_path), None)
